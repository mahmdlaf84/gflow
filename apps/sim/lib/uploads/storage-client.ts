...
import type { CustomBlobConfig } from '@/uploads/blob/blob-client'
import type { CustomS3Config } from '@/uploads/s3/s3-client'
import { USE_BLOB_STORAGE, USE_S3_STORAGE } from '@/uploads/setup'
...
export async function uploadFile(
  ...
  const { UPLOAD_DIR_SERVER } = await import('@/uploads/setup.server')
...
export async function downloadFile(
  ...
  if (USE_BLOB_STORAGE) {
    ...
    const { downloadFromBlob } = await import('@/uploads/blob/blob-client')
    if (customConfig) {
      const blobConfig: CustomBlobConfig = {
  ...
  if (USE_S3_STORAGE) {
    ...
    const { downloadFromS3 } = await import('@/uploads/s3/s3-client')
    if (customConfig) {
      const s3Config: CustomS3Config = {
...
export async function deleteFile(key: string): Promise<void> {
  if (USE_BLOB_STORAGE) {
    ...
    const { deleteFromBlob } = await import('@/uploads/blob/blob-client')
    return deleteFromBlob(key)
  }

  if (USE_S3_STORAGE) {
    logger.info(`Deleting file from S3: ${key}`)
    const { deleteFromS3 } = await import('@/uploads/s3/s3-client')
...
export async function getPresignedUrl(key: string, expiresIn = 3600): Promise<string> {
  if (USE_BLOB_STORAGE) {
    ...
    const { getPresignedUrl: getBlobPresignedUrl } = await import('@/uploads/blob/blob-client')
    return getBlobPresignedUrl(key, expiresIn)
  }

  if (USE_S3_STORAGE) {
    logger.info(`Generating presigned URL for S3: ${key}`)
    const { getPresignedUrl: getS3PresignedUrl } = await import('@/uploads/s3/s3-client')
